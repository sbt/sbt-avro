...
lazy val `transitive`: Project = project
  ...
  .settings(
    name := "transitive",
    version := "0.0.1-SNAPSHOT",
    libraryDependencies += ("com.github.sbt" % "external" % "0.0.1-SNAPSHOT" % "avro")
      .classifier("avro")
      .intransitive()
  )
...
lazy val app: Project = project
  ...
  .settings(
    ...
    Compile / checkGenerated := {
      ...
      exists(
        (`transitive` / crossTarget).value / "src_managed" / "compiled_avro" / "main" / "com" / "github" / "sbt" / "avro" / "test" / "external" / "Avsc.java"
      )
      exists(
        (`transitive` / crossTarget).value / "src_managed" / "compiled_avro" / "main" / "com" / "github" / "sbt" / "avro" / "test" / "transitive" / "Avsc.java"
      )
      ...
      exists(
        crossTarget.value / "src_managed" / "compiled_avro" / "main" / "com" / "github" / "sbt" / "avro" / "test" / "external2" / "Avsc.java"
      )

      // Check that transitive2 has been recompiled in `app` project
      exists(
        crossTarget.value / "src_managed" / "compiled_avro" / "main" / "com" / "github" / "sbt" / "avro" / "test" / "transitive2" / "Avsc.java"
      )
    }
  )
